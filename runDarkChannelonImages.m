function [ score ] = runDarkChannelonImages()

% LI
% calcualtes dark channel measure on images
% baseImageDir must contain the real blurry images 
% deblurredDir dataset must contain the deblurred images
...
% initialise score
% dataset does not contain 1400, this is slightly bigger some images missing
score = zeros (1,1400);
iter = 1;
...
for i = 1:length(imageNamesList) 
    ...
    try
        score(iter) = darkChannelAverage(blurred, blurred, 35, 4);
    catch
%         score(iter) = NaN;
%         details(iter) = NaN;
    end
    ...
    for j = 1:length(methodNamesList)
        ...
        if exist(imageFile{1}, 'file')~= 2
            ...
            try
                score(iter) = darkChannelAverage(deblurred, blurred, 35, 4);
            catch
%                 score(iter) = NaN;
%                 details(iter) = NaN;
            end
