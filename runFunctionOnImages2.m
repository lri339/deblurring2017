function [ score ] = runFunctionOnImages2(~)
%RUNFUNCTIONONIMAGES 
% LI
% calcualte the ringing metric on all images
% uses CVPR 2016 review study dataset
...
for i = 1:length(imageNamesList) 
    ...
    try
        %[score(iter)] = ringingFeatureSimple(blurred,blurred);
        %[score(iter)] = contrastMetric(blurred);
        [score(iter)] = ringingFeature(blurred,blurred);
    catch
%         score(iter) = NaN;
%         details(iter) = NaN;
    end
    ...
    for j = 1:length(methodNamesList)
        ...
        if exist(imageFile{1}, 'file')~= 2
            ...
            %try
                %[score(iter)] = ringingFeatureSimple(deblurred,blurred);
                %[score(iter)] = contrastMetric(deblurred);
                score(iter) = ringingFeature(blurred, deblurred);
            %catch
%                 score(iter) = NaN;
%                 details(iter) = NaN;
            %end
